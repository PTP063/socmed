...
def get_posts(db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user),limit: int = 10, skip: int = 0, search: Optional[str] = ""):
    posts = db.query(models.Post).filter(models.Post.title.contains(search)).limit(limit).skip(skip).all()
    print (limit)
    return {"data": posts}
...
def get_post(id: int, db: Session = Depends(get_db)):
    ...
    if not post:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"post with id: {id} does not exist")
    results=db.query(models.Post, models.User).join(models.User, models.Post.owner_id == models.User.id).filter(models.Post.id == id).first()
