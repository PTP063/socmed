...
export const Register: React.FC<RegisterProps> = ({ onNavigate }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const loginRes = await api.post<AuthResponse>('/auth/login', formData);
    ...
    <div className="max-w-md mx-auto pt-8">
      <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 rounded-3xl p-8 shadow-xl space-y-6">
        <div className="text-center space-y-2">
          ...
          <h2 className="text-2xl font-black text-slate-900 dark:text-slate-100">Join SocMed</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400">Create your account in seconds</p>
        </div>
